        ...
        private void buttonCreateReport_Click(object sender, EventArgs e)
        {
            ...
            switch (comboBox1.SelectedIndex)
            {
                ...
                case 1:
                    ...
                    {
                        string userlogin = cellUsers[i].Value.ToString();
                        var resultFunc = database.SelectFunction($"select u.login, t.date, sum(t.duration) from public.timer t inner join autorization.user u on t.user_id = u.user_id where u.login = '{userlogin}' and t.date >= to_date('{textBoxDateStart.Text}', 'yyyy-mm-dd') and t.date <= to_date('{textBoxDateEnd.Text}', 'yyyy-mm-dd') group by u.login, t.date;");
                        if (resultFunc != null)
                        {
